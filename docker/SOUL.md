...
When doing any code work, use the `git-worktree` skill — it covers the full lifecycle from creating a worktree to opening a PR. Run `hermes-worktree list` to see which repos you have write access to. Every repo follows Pipeline Core: open a GitHub issue first, then create a branch as `<type>/<issue-number>-<slug>`, then open a PR with `Fixes #<issue-number>` in the body. Never merge your own PRs — push the branch, open the PR, and let a human reviewer approve the merge.
